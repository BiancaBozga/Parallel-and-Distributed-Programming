...
private:
    int matrixElemExtended(int i, int j) const {
        //setam i pe primul rand/ultimul
        //setam j pe prima coloana sau ultima
        if (i < 0) {
            i = 0;
        }
        else if (i >= N) {
...
protected:
    int oneConv(int i, int j) const {
        int sum = 0;
        for (int p = 0; p < K; p++) {
            //indexul rândului în matricea de intrare care corespunde rândului curent al kernel-ului.
            int idx = i + p - K / 2;
            for (int q = 0; q < K; q++) {
                //calculează indexul coloanei în matricea de intrare corespunzător coloanei curente a kernel-ului:j + q - K / 2
...
public:
    ...
    void apply() const override {
        ...
        int linesLeft = N % P;
        //parcurgem liniile cu pas=nr de linii pt fiecare thread
        for (int k = 0; k < N; k += linesPerThread) {
            ...
            int end = k + linesPerThread - 1;//indexul final
...
{    int num = std::stoi(argv[5]);
    ...
    cout << setprecision(5)<<elapsedTimeMs << endl;
